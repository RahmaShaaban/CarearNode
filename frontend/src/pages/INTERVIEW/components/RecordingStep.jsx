...
const CameraOverlay = memo(({ status, stopRecording, resetKey, primaryColor }) => {
    const [timeLeft, setTimeLeft] = useState(60);
    ...
    return (
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, pointerEvents: 'none', backgroundColor: 'rgba(0,0,0,0.15)', transform: 'translateZ(0)' }}>
            ...
            <div style={{ position: 'absolute', bottom: '20px', width: '100%', display: 'flex', justifyContent: 'center' }}>
                <span style={{ background: primaryColor, color: '#ffffff', padding: '8px 20px', borderRadius: '25px', fontSize: '0.9rem', fontWeight: '500', boxShadow: '0 4px 6px rgba(0,0,0,0.2)', display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="16" x2="12" y2="12"></line><line x1="12" y1="8" x2="12.01" y2="8"></line></svg>
                    Keep your upper body within the frame
                </span>
            </div>
        </div>
    );
...
const RecordingStep = ({ questions, currentQuestionIndex, onNext, onFinish, onBack }) => {
    ...
    const processVideoAndProceed = async (actionType) => {
        ...
        try {
            ...
            const videoFile = new File([blob], `answer_${currentQuestionIndex}.webm`, { type: "video/webm" });
            
            if (actionType === 'next') {
                await onNext(videoFile); 
                clearBlobUrl(); 
                setRetakeKey(prev => prev + 1); 
            } else {
                await onFinish(videoFile); 
            }
        } catch (error) {
    ...
    return (
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '20px', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
          ...
          <div style={{ display: 'flex', gap: '30px', alignItems: 'stretch', height: '600px', marginBottom: '20px' }}>
                
    <div style={{ flex: '1', display: 'flex', flexDirection: 'column', justifyContent: 'center', textAlign: 'left', paddingTop: '10px', paddingBottom: '10px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ background: '#ffffff', padding: '20px', borderRadius: '16px', border: `2px solid ${primaryColor}`, boxShadow: '0 4px 15px rgba(88, 164, 146, 0.1)', marginBottom: '15px', boxSizing: 'border-box' }}>
                {/* 🚀 تم إضافة unicodeBidi و direction و textAlign هنا لحل مشكلة قراءة النص العربي والإنجليزي معاً */}
                <h2 style={{ color: darkGreen, fontSize: '1.4rem', lineHeight: '1.4', fontWeight: '700', margin: 0, direction: 'rtl', textAlign: 'right', unicodeBidi: 'plaintext' }}>
                    "{questions[currentQuestionIndex]}"
                </h2>
            </div>
            
            <div style={{ background: '#ffffff', border: 'none', padding: '15px 20px', borderRadius: '12px', boxShadow: '0 4px 20px rgba(47, 93, 84, 0.08)', boxSizing: 'border-box', direction: 'ltr' }}>
                ...
                <ul style={{ margin: 0, paddingInlineStart: '25px', color: darkGreen, fontSize: '0.9rem', lineHeight: '1.5', fontWeight: '500' }}>
                    <li style={{ marginBottom: '6px' }}>You have exactly 1 minute to answer.</li>
                    <li style={{ marginBottom: '6px' }}>Look directly at the camera while answering.</li>
                    <li>Use hand gestures naturally to explain your points.</li>
                </ul>
            </div>
        </div>
                    ...
                    Start Answer (1 Min)
                        ...
                        {status === 'stopped' && (
                            <div style={{ display: 'flex', gap: '12px' }}>
                                <button onClick={handleRetake} disabled={isProcessing} style={{ flex: '1', padding: '14px 10px', background: '#ffffff', color: darkGreen, border: `2px solid ${primaryColor}`, borderRadius: '12px', cursor: isProcessing ? 'not-allowed' : 'pointer', fontWeight: '700', fontSize: '1rem', transition: 'all 0.2s', boxSizing: 'border-box', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '6px' }} onMouseOver={(e) => { if(!isProcessing) e.currentTarget.style.backgroundColor = bgColor; }} onMouseOut={(e) => { if(!isProcessing) e.currentTarget.style.backgroundColor = '#ffffff'; }}>
                                ...
                                <button 
                                    onClick={() => processVideoAndProceed(isLastQuestion ? 'finish' : 'next')} 
                                    disabled={isProcessing} 
                                    style={{ 
                                        flex: '2', padding: '14px 10px', fontSize: '1rem', fontWeight: '700', 
                                        backgroundColor: isLastQuestion ? darkGreen : primaryColor, 
                                        color: '#fff', border: 'none', borderRadius: '12px', 
                                        cursor: isProcessing ? 'wait' : 'pointer', display: 'flex', 
                                        justifyContent: 'center', alignItems: 'center', gap: '8px', 
                                        opacity: isProcessing ? 0.7 : 1, transition: 'all 0.2s', boxSizing: 'border-box' 
                                    }} 
                                    onMouseOver={(e) => { if(!isProcessing) e.currentTarget.style.backgroundColor = isLastQuestion ? '#1f4039' : darkGreen }} 
                                    onMouseOut={(e) => { if(!isProcessing) e.currentTarget.style.backgroundColor = isLastQuestion ? darkGreen : primaryColor }}
                                >
                                    {isProcessing ? (
                                        <>
