...
# --- استيراد الملفات ---
from master_vision import analyze_vision_single_pass
...
async def api_analyze_all(file: UploadFile = File(...), question: str = Form(...), language: str = Form(...)):
    ...
    try:
        # 1. تشغيل تحليل الصوت أولاً ثم الرؤية (بالتتابع لمنع اختناق الـ CPU وتسريع المعالجة)
        speech_res = await asyncio.to_thread(analyze_audio_features, path, language)
        vision_res = await asyncio.to_thread(analyze_vision_single_pass, path)
