...
try:
    attire_model = YOLO('Interview_Attire/run_final_model/weights/best.pt')
...
def analyze_vision_single_pass(video_path):
    ...
    while cap.isOpened():
        ...
        # =======================================
        if attire_model and attire_frames_checked < 30:
    # نصغر الصورة لـ 224 عشان تبقى سريعة جداً مع موديل التصنيف
            frame_224 = cv2.resize(frame, (224, 224))
            
            # ندي الفريم للموديل
            results_cls = attire_model(frame_224, verbose=False)
            
            # استخراج النتيجة
            result = results_cls[0]
            probs = result.probs
            highest_class_idx = probs.top1
            class_name = result.names[highest_class_idx] # هيطلع 'formal' أو 'informal'
            confidence = float(probs.top1conf) # نسبة ثقة الموديل في قراره
            
            # لو الموديل قال إنه رسمي (formal) وواثق بنسبة أكبر من 60%
            if class_name == 'formal' and confidence > 0.60:
