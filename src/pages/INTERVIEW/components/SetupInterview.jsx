...
const SetupInterview = ({ formData, setFormData, setQuestions, setStep, isArabic, setInterviewId }) => {
    const [loading, setLoading] = useState(false);
    ...
    const handleStartInterview = async () => {
        ...
        try {
            // 💡 سحب الـ ID من الـ LocalStorage (القيمة اللي شفناها في الصورة هي 16)
            const userIdFromStorage = localStorage.getItem('userId');

            // 1️⃣ الخطوة الأولى: نكلم Node.js (5001) لإنشاء سجل المقابلة في الداتابيز
            ...
            const dbRes = await axios.post('http://localhost:5001/api/interview/start', {
                userId: userIdFromStorage, // 👈 إرسال الـ ID لملء خانة الـ user_id في جدول interviews
            ...
            if (dbRes.data.success) {
                setInterviewId(dbRes.data.interviewId); // حفظ الـ ID في الـ App State
                console.log("Interview ID created:", dbRes.data.interviewId);
            }

            // 2️⃣ الخطوة الثانية: نكلم Python (5000) لتوليد الأسئلة بالذكاء الاصطناعي
            const form = new FormData();
            form.append('track', formData.track);
            form.append('level', formData.level);
            form.append('language', formData.language);
            ...
            alert(isArabic ? "فشل في بدء المقابلة. تأكد من تشغيل السيرفرات (5000 و 5001)." : "Error starting interview. Check backends (5000 & 5001).");
            console.error(err);
        } finally {
            setLoading(false);
        }
    ...
    // 🌟 مكون القائمة المنسدلة المخصص (Custom Select) 
    const CustomSelect = ({ value, options, onChange }) => {
        const [isOpen, setIsOpen] = useState(false);
        const dropdownRef = useRef(null);
    ...
    return (
        <div style={{ minHeight: '100vh', padding: '40px 20px', fontFamily: 'system-ui, -apple-system, sans-serif', textAlign: isArabic ? 'right' : 'left', direction: isArabic ? 'rtl' : 'ltr' }}>
            <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
                <div style={{ marginBottom: '40px', textAlign: 'center' }}>
                    <h1 style={{ color: darkGreen, fontSize: '2.5rem', fontWeight: '800', marginBottom: '15px' }}>
                        {isArabic ? "مدرب المقابلات الذكي" : "AI Interview Coach"}
