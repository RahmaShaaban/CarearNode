...
const RecordingStep = ({ questions, currentQuestionIndex, isArabic, onNext, onFinish, onBack }) => {
    ...
    // ✨ دالة معالجة الفيديو وإرساله للأب (App.js)
    const processVideoAndProceed = async (actionType) => {
        if (!mediaBlobUrl) return;
        setIsProcessing(true);
        
        try {
            // تحويل الـ Blob URL إلى ملف حقيقي
            const response = await fetch(mediaBlobUrl);
            const blob = await response.blob();
            
            // ننشئ ملف فيديو (بصيغة webm لأنها الأصلية للمتصفح وأخف في الرفع)
            const videoFile = new File([blob], `answer_${currentQuestionIndex}.webm`, { type: "video/webm" });
            
            if (actionType === 'next') {
                await onNext(videoFile); // استدعاء دالة الرفع في App.js
                clearBlobUrl(); 
                setRetakeKey(prev => prev + 1); 
            } else {
                await onFinish(videoFile); // استدعاء دالة الإنهاء والرفع في App.js
            }
        } catch (error) {
            console.error("Error processing video:", error);
            alert(isArabic ? "حدث خطأ أثناء معالجة الفيديو." : "Error processing video.");
        } finally {
            setIsProcessing(false);
        }
    };
                            ...
                            <div style={{ display: 'flex', gap: '12px', flexDirection: isArabic ? 'row-reverse' : 'row' }}>
                                ...
                                >
                                    {isProcessing ? (
                                        ...
                                        <>
                                            {isLastQuestion ? (isArabic ? "إنهاء وتحليل" : "Finish & Analyze") : (isArabic ? "السؤال التالي" : "Next Question")}
                                            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ transform: isArabic ? 'rotate(180deg)' : 'none' }}><line x1="5" y1="12" x2="19" y2="12"></line><polyline points="12 5 19 12 12 19"></polyline></svg>
                                        </>
                                    )}
                                </button>
